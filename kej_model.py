...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # #   conv1
        # self.conv1 = nn.Conv1d(FINAL_DIM, filter_num, kernel_size=filter_size, padding=1)
        #
        # #   maxpool1
        # self.mp1 = nn.MaxPool1d(filter_num)



    def forward(self, tokenMatrix, pos1Matrix, pos2Matrix):
        word_input = self.word_embeds(tokenMatrix)
        pos1_input = self.pos1_embeds(pos1Matrix)
        pos2_input = self.pos2_embeds(pos2Matrix)

        final_input = torch.cat((word_input, pos1_input, pos2_input), dim=2)
...
# model = Net()
model = torch.load("pkl/kejmodel.torch")
print(model)
...
def train(epoch,data_loader):
    ...
    for batch_idx, (ys, tokenMatrix, pos1Matrix, pos2Matrix, sdpMatrix) in enumerate(data_loader):
        ...
        output = model(tokenMatrix, pos1Matrix, pos2Matrix)
...
torch.save(model,"pkl/kejmodel.torch")